...
class MainActivity : AppCompatActivity(), CallLogAdapter.OnItemChangedListener {
    ...
    private fun refreshCallLogs(positions: List<Int> = listOf()) {
        lifecycleScope.launch {
            val blockedNumbers = withContext(Dispatchers.IO) { getBlockedNumbers(this@MainActivity) }
            val whitelistNumbers = withContext(Dispatchers.IO) { getWhitelistNumbers(this@MainActivity) }
            val callLogs = withContext(Dispatchers.IO) { getCallLogs(this@MainActivity) }

            if (callLogAdapter == null) {
                callLogAdapter = CallLogAdapter(this@MainActivity, callLogs, blockedNumbers, whitelistNumbers)
                recyclerView?.adapter = callLogAdapter
                callLogAdapter?.setOnItemChangedListener(this@MainActivity)
            } else {
